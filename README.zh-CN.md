...
插件以子进程脚本运行,或接入 MCP 服务,权限在清单里声明。可以从内置市场装,也可以给一个 zip 地址 ——**落地之前**先把包下下来读一遍清单,把它声明的权限和会带来的工具摊开给你看。插件还能交出**文件**(JSON 通道搬不动大文件,所以要么写进给它的暂存目录,要么交出下载凭据让宿主去取),以及**记住**跨调用的东西(比如续期出来的 OAuth 令牌 —— 宿主按清单声明的键存好,下次注入回环境变量)。后端也说你的语言 —— 任务消息、
