...
到这儿就能进去随便看了。真要生成或转写,至少得先配一条供应商连接 —— 设置 → 供应商。连接是
当前用户自己的端点，秘密/OAuth 状态单独保存，模型挂在它下面。
...
视频素材右键可选「**转为 GIF**」：产出是一条新的派生素材，原视频永远不覆盖；需要批处理时，
工作流里有同名节点。
...
Base URL:无工具 Gateway 分析采样帧;只有明确支持的 API Adapter 才直读整段原生视频。OAuth 选择
「原生」会明确报错并建议改用抽帧,不会偷偷换模型或分析方式。
...
连接和模型分两级:一条**供应商连接**是当前用户自己的端点与秘密/OAuth 状态,下面可以挂任意多个模型。能力
(对话 / 绘图 / 视频 / 音频)、上下文长度、推理与视觉开关,都属于**模型**。订阅制供应商还能
查剩余额度和重置周期。

Evolink 按「统一图像/视频网关」接入：只配一份 Evolink API Key，再在连接下面添加 Seedance、
Kling、Veo、海螺、WAN、Sora、GPT Image、Gemini、Seedream 等模型。引用本地图片时先走
Evolink Files API，视频与音频参考也走同一上传入口但保留首尾帧、参考、编辑源、续写片段等语义；
任务完成后立即下载回本地素材库，避免结果限时地址过期。生成控件只读取精确型号的能力描述符，
手动添加的未知模型仍可使用，但不会继承另一型号猜出来的时长、分辨率或输入规则。

字节跳动原生接入按产品协议归位：方舟 Ark 下是 Seedream / Seedance 生成，火山语音下是 TTS 与
播客 WebSocket。它们同属字节跳动，但控制台、凭据和协议不可互换；已有连接 id 保持兼容。
