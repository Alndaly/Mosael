...
#: YouTube 拿哪个客户端的身份去取流。**默认那个现在会被 403** —— 实测同一条视频,不带这个参数
#: 直接 `HTTP Error 403: Forbidden`,换成 android 立刻 5 MB/s 下完。
#:
#: 这是一串**会过期的经验值**,不是常量:站点变、yt-dlp 也在追。写成列表是因为 yt-dlp 会按顺序
#: 依次试,前一个不行自动换下一个 —— 哪天默认恢复正常了,这一行删掉即可,而不是留着一个
#: 越来越旧的单一选择。只对 youtube 这个 extractor 生效,别的站点不受影响。
_YOUTUBE_CLIENTS = ["android", "web_safari", "web"]
...
def probe(url: str, *, cookie_file: Path | None = None) -> RemoteListing:
