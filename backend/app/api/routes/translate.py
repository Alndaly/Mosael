...
from app.domain.translate import TranslateError, translate
...
def translate_texts(body: TranslateRequest, db: DbSession, user: CurrentUser) -> dict:
    ...
    try:
        out = [
            translate(db, text, body.target_lang, engine=body.engine, profile_id=body.profile_id) if text.strip() else ""
