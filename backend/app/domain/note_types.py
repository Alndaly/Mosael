...
class NoteSource(BaseModel):
    ...
    def valid_range(self):
        if self.end is not None and (self.start is None or self.end <= self.start):
            raise PydanticCustomError("note_range", tr("noteErr_rangeOrder"))
        if self.kind == "url":
            from urllib.parse import urlsplit
            try:
                parsed = urlsplit(self.url)
            except ValueError as exc:
                raise PydanticCustomError("note_url", tr("noteErr_badUrl")) from exc
            if parsed.scheme not in ("http", "https") or not parsed.hostname:
                raise PydanticCustomError("note_url", tr("noteErr_urlScheme"))
        elif not self.id:
            raise PydanticCustomError("note_source", tr("noteErr_sourceIdEmpty"))
        return self
