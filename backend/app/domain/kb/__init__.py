...
"""
知识库内核(计划 §6.9,借鉴 Revornix 的"一切转 markdown → 分块 → 检索"
管线,但坚持 local-first:检索底座是 SQLite FTS5 的 trigram 分词
(对中文按三字子串索引,无需 jieba/向量库),零外部服务依赖。
向量/摘要/图谱是后续增量,不进 v1 内核。
"""
...
def reindex_document(db: Session, document: KbDocument) -> int:
    """重建该文档的 chunk 与 FTS 行;返回 chunk 数。"""
    _ensure_fts(db)
...
def search(db: Session, workspace_id: str, query: str, *, limit: int = 8) -> list[dict[str, Any]]:
    """FTS5 trigram 检索(bm25 排序),短查询回退 LIKE;每篇文档只留最佳块。"""
    cleaned = query.strip()
    if not cleaned:
        return []
    _ensure_fts(db)
    rows: list[tuple[str, str, float]] = []  # (chunk_id, document_id, score 越小越好)
    if len(cleaned) >= MIN_FTS_QUERY_CHARS:
        fts_query = '"' + cleaned.replace('"', '""') + '"'
        rows = [
            (row[0], row[1], float(row[2]))
            for row in db.execute(
                sa_text(
                    "SELECT chunk_id, document_id, rank FROM kb_chunks_fts "
                    "WHERE workspace_id = :ws AND kb_chunks_fts MATCH :q ORDER BY rank LIMIT :limit"
                ),
                {"ws": workspace_id, "q": fts_query, "limit": limit * 4},
            )
        ]
    if not rows:
        like = f"%{cleaned}%"
        rows = [
            (chunk.id, chunk.document_id, 0.0)
            for chunk in db.scalars(
                select(KbChunk)
                .where(KbChunk.workspace_id == workspace_id, KbChunk.text.like(like))
                .limit(limit * 4)
            )
        ]
    ...
    for chunk_id, document_id, score in rows:
        if document_id not in best_per_doc:
