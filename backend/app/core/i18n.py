...
MESSAGES: dict[str, dict[str, str]] = {
    ...
    "wfField_engine_voice": {"zh": "音色", "en": "Voice"},
    "wfField_engine_voice_resource": {"zh": "音色资源号", "en": "Voice resource id"},
    ...
    "wfNode_synthesize_speech_voice_id": {"zh": "配音库里克隆好的那把嗓子", "en": "One of the voices you cloned into the voice library"},
    "wfNode_synthesize_speech_engine": {"zh": "嗓子从哪来:用配音库里克隆的,还是某个引擎现成的", "en": "Where the voice comes from: one you cloned, or a stock voice from an engine"},
    "wfNode_synthesize_speech_engine_voice": {"zh": "该引擎下的具体音色", "en": "Which voice within that engine"},
    "wfNode_synthesize_speech_engine_voice_resource": {"zh": "少数引擎(火山)的音色还带一个资源号,选音色时自动填上", "en": "A few engines (Volcano) pair the voice with a resource id; picking a voice fills this in"},
    ...
    "wfNode_separate_audio_engine": {"zh": "用哪个分离引擎。auto = 用本机现在装好的那个;引擎在「设置 → 本机引擎 → 人声分离」里装。", "en": "Which separation engine to use. auto uses whichever is installed on this machine; install engines under Settings → On-device engines → Voice separation."},
    ...
    "wfNode_denoise_audio_engine": {
        "zh": "用哪个降噪引擎。auto = 内置的频谱降噪(不装任何东西,不动音乐,只去持续的底噪);deepfilternet = 效果最好的语音降噪(要先在设置里下载);rnnoise = 轻量语音降噪。后两种都会把音乐一起去掉。",
        "en": "Which engine to use. auto is the built-in spectral denoiser (nothing to install, music untouched, steady noise only); deepfilternet is the best speech denoiser (download it in Settings first); rnnoise is a lightweight speech denoiser. The last two remove music as well.",
    },
    "wfNode_denoise_audio_strength": {
        "zh": "下手多重:light 只去最明显的底噪,medium 适合大多数录音,strong 去得最干净但可能让声音发闷。",
    ...
    "wfNode_dub_subtitles_original_audio": {
        "zh": "配音之后原声怎么办:duck=在配音说话时压低到 30%(适合原声是环境音/音乐);mute=成片里不出现(译配用这个,两边都是人声,压低只会变成两个人同时说话);keep=原样留着;separate=先把原声拆成人声和背景音,只丢人声、留住背景(需要本机装了分离引擎,没装就退回 mute)。都不删东西,轨上那个开关随时能改回来。",
        "en": "What happens to the original audio once the dub lands: duck lowers it to 30% while the dub speaks (right when the original is ambience or music); mute keeps it out of the export (use this for translated dubbing \u2014 both sides are speech, and ducking just gives you two people talking at once); keep leaves it alone; separate splits the original into voice and background first, so only the voice goes and the background stays (needs a separation engine on this machine; falls back to mute without one). Nothing is deleted; the track switch can be flipped back at any time.",
    },
