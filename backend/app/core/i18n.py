...
MESSAGES: dict[str, dict[str, str]] = {
    ...
    "confirmErr_runBoardItemNotTool": {
        "zh": "「{item_id}」不是工具格或 3D 场景格:智能体只能运行这两种;图片/视频/音频槽和便签由用户在面板上生成",
        "en": "“{item_id}” is not a tool item or a 3D scene item. The agent can only run those two; image/video/audio slots and notes are generated by the user from their panel.",
    },
    ...
    "boardNote_toolFetchesByExternalId": {"zh": "「{tool}」这一步已经不在画板上了:它按另一个系统里的编号(任务号、文件 id、对象路径)把东西取回来,画板只放把内容变成新内容的工具 —— 需要的话在工作流里用它,要把文件放上画板就从素材库拖进来。", "en": "“{tool}” is no longer on the board: it fetches things by an id from another system (a task id, a file id, an object key), and the board only holds tools that turn content into new content. Use it in a workflow if you need it; to put a file on the board, drag it in from the asset library."},
    "boardNote_originalSettings": {"zh": "原来的设置:", "en": "The original settings:"},
    ...
    "boardErr_formNeedsTool": {"zh": "画板项 {item_id} 的表单要写明跑哪个工具(producer 写成 node:<节点类型>,3D 场景格是 scene_render,见 list_board_producers),收到的是「{producer}」", "en": "Board item {item_id}: the form must name a tool to run (producer as node:<node type>, or scene_render on a 3D scene item; see list_board_producers); got “{producer}”."},
    "boardErr_formOnlyOnAction": {"zh": "只有工具格(action)和 3D 场景格(scene)的表单能这样写,画板项 {item_id} 是 {kind}", "en": "Only a tool item (action) or a 3D scene item (scene) takes a form this way; board item {item_id} is a {kind}."},
