...
MESSAGES: dict[str, dict[str, str]] = {
    ...
    "confirm_boardItemNamed": {"zh": "(格子「{name}」)", "en": " (item “{name}”)"},
    "confirm_boardRunVia": {"zh": ",用你的连接「{name}」", "en": ", using your connection “{name}”"},
    #: 卡上点明「为什么要你看一眼」的那半句,按后果分(词表见 domain/effects)。画板上的能力和插件工具共用。
    ...
    "wfNode_video_to_gif_board": {"zh": "把一段视频做成 GIF 动图", "en": "Turn a video clip into an animated GIF."},
    ...
    "wfNode_separate_audio_board": {"zh": "把音频或视频拆成人声和背景音两份", "en": "Split audio or video into a voice stem and a background stem."},
    ...
    "wfNode_denoise_audio_board": {"zh": "去掉音频或视频里的底噪,另存一份干净的", "en": "Clean the background noise out of audio or video as a new copy."},
    ...
    "boardErr_nodeNotOnBoard": {"zh": "「{node}」不能在画板上跑:画板上只放把内容变成新内容的工具,流程控制和数据处理请在工作流里做", "en": "“{node}” can't run on a board: a board only holds tools that turn content into new content. Do flow control and data handling in a workflow."},
    "boardErr_toolNotOnBoard": {"zh": "「{tool}」不上画板:它不交出素材或点名落板的文字,不是把内容变成新内容的工具。请在工作流里用它", "en": "“{tool}” isn't a board tool: it doesn't hand back assets or text meant for the board, so it doesn't turn content into new content. Use it in a workflow."},
    "boardErr_toolFetchesByExternalId": {"zh": "「{tool}」不上画板:它按另一个系统里的编号(任务号、文件 id、对象路径)把东西取回来,不是把画板上的内容变成新内容。请在工作流或对话里用它;要把文件放上画板,从素材库拖进来", "en": "“{tool}” isn't a board tool: it fetches things by an id from another system (a task id, a file id, an object key) rather than turning content on the board into new content. Use it in a workflow or in chat; to put a file on the board, drag it in from the asset library."},
    "boardErr_toolMirroredByGeneration": {"zh": "「{tool}」和生成模型「{model}」是同一件事:画板上放一格图片 / 视频 / 音频、选这个模型来生成(工作流里照样可以用这个工具)", "en": "“{tool}” is the same thing as the generation model “{model}”: on a board, add an image / video / audio item and generate with that model (the tool is still available in workflows)."},
    "pluginNode_mirroredByGeneration_image": {"zh": "只要图片的话,用「AI 生成素材」节点选这个模型(同一张工作流,还有回执、用量和更长的等待)。", "en": "If you only need the image, use the “AI generate” node with this model (same workflow, plus receipts, usage and a longer wait)."},
    "pluginNode_mirroredByGeneration_video": {"zh": "只要视频的话,用「AI 生成素材」节点选这个模型(同一张工作流,还有回执、用量和更长的等待)。", "en": "If you only need the video, use the “AI generate” node with this model (same workflow, plus receipts, usage and a longer wait)."},
    "pluginNode_mirroredByGeneration_audio": {"zh": "只要音频的话,用「AI 生成素材」节点选这个模型(同一张工作流,还有回执、用量和更长的等待)。", "en": "If you only need the audio, use the “AI generate” node with this model (same workflow, plus receipts, usage and a longer wait)."},
    "boardToolGroup_new": {"zh": "产出新素材", "en": "New assets"},
    "boardToolGroup_image": {"zh": "处理图片", "en": "Work on images"},
    "boardToolGroup_video": {"zh": "处理视频", "en": "Work on video"},
    "boardToolGroup_audio": {"zh": "处理音频", "en": "Work on audio"},
    "boardToolGroup_text": {"zh": "处理文字", "en": "Work on text"},
    "boardToolGroup_scene": {"zh": "处理 3D 场景", "en": "Work on 3D scenes"},
    "boardToolGroup_asset": {"zh": "处理素材", "en": "Work on assets"},
    ...
    "boardErr_formNeedsTool": {"zh": "画板项 {item_id} 的表单要写明产出者:一项能力或空格子上的生成器写成 node:<节点类型>,3D 场景格是 scene_render(见 list_board_producers),收到的是「{producer}」", "en": "Board item {item_id}: the form must name its producer — node:<node type> for an ability or an empty slot's generator, or scene_render on a 3D scene item (see list_board_producers); got “{producer}”."},
    "boardErr_formNotOnKind": {"zh": "画板项 {item_id} 是 {kind},这种格子没有能这样写的表单:能力的设置写在它挂着的内容格上(set_form 带上那一项的 producer)", "en": "Board item {item_id} is a {kind}, which has no form to write this way. An ability's settings go on the content item it belongs to (set_form with that ability's producer)."},
    "boardErr_abilityNeedsContent": {"zh": "「{item_id}」还没有内容,「{tool}」没有东西可处理:先放进素材、写上字或挑一篇笔记", "en": "“{item_id}” has no content yet, so “{tool}” has nothing to work on. Put an asset in it, write some text or pick a note first."},
    "boardErr_abilityNotOnKind": {"zh": "「{producer}」不是{kind}格子的一项能力(它挂在:{kinds})", "en": "“{producer}” isn't an ability of a {kind} item (it belongs on: {kinds})."},
    "boardErr_abilityIsNotAProducer": {"zh": "「{producer}」是{kind}格子的一项能力,不能当画板项 {item_id} 自己的产出者:用 set_form 写它的设置、run_board_item 带上 producer 跑它", "en": "“{producer}” is an ability of a {kind} item, not item {item_id}'s own producer. Write its settings with set_form and run it with run_board_item, passing producer."},
    ...
    "boardErr_bindingNotWired": {"zh": "字段「{field}」接的「{source}」没有连到「{item_id}」:先用 connect 连一根从它到这一格的线", "en": "Field “{field}” is bound to “{source}”, but there is no edge from it to “{item_id}”. Connect them first."},
    ...
    "boardProducer_scene_render": {"zh": "渲白模参考", "en": "Render references"},
    "boardProducer_scene_render_desc": {"zh": "选一个镜头,渲出首尾帧或运镜视频", "en": "Pick a shot and render its first and last frames or its camera move."},
