...
def test_档案填的是对话端点时_语音要归一到原生根() -> None:
    ...
    """
    from app.audio.tts import DASHSCOPE_NATIVE_BASE, resolve_dashscope_native_base
...
def test_构造签名要收voice() -> None:
    ...
    from app.audio.tts import build_remote_provider
...
def test_认不出的模型回空_由界面退回填id() -> None:
    ...
    from app.audio.tts import CosyVoiceTTS
...
def test_cosyvoice走另一个端点和另一种请求体() -> None:
    ...
    """
    from app.audio.tts import SpeechRequest
...
def test_播客音色不能显示成原始id() -> None:
    ...
    from app.audio.tts import EDGE_BUILTIN_VOICES, PODCAST_SPEAKERS, VOLCANO_BUILTIN_VOICES
...
def test_两个引擎共用同一条连接的凭据() -> None:
    ...
    from app.audio.tts import CosyVoiceTTS, vendor_for_engine
...
def test_模型按族筛_免得把qwen的模型发去cosyvoice() -> None:
    ...
    from app.audio.tts import BailianTTS as B, CosyVoiceTTS as C
...
def test_两个引擎各自的音色和语速() -> None:
    ...
    from app.audio.tts import CosyVoiceTTS, describe_engines
