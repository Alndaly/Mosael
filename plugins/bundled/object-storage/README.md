...
| 工具 | 干什么 |
| --- | --- |
| `storage_upload` | 传一份素材上去,交回限时直链 + 公共读地址。**流式**:大文件分片上传、边传边报进度,取消时停在片与片之间并清掉已传的分片 |
| `storage_presign` | 给桶里已有的对象签一条限时直链 |
| `storage_fetch` | 把桶里的对象拉回素材库(**交地址,由宿主搬字节** —— 进度、取消、重试都归它) |
| `storage_list` | 列出某个前缀下的对象,一页最多 1000 条,还有下一页时交回 `next_cursor` |
