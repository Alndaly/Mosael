...
| 工具 | 干什么 |
| --- | --- |
| `pan_list` | 列一个目录下的文件和子目录,给出 `fs_id`;没列完时交回 `has_more` 和 `next_start` |
| `pan_search` | 按文件名搜(递归),找不准路径时用它;命中比 `limit` 多时 `has_more` 为真 |
| `pan_import` | 按 `fs_id` 把文件拉进素材库,返回 `asset_id` |
| `pan_upload` | 把素材库里的一个文件传到网盘,返回 `fs_id`;边传边报进度,可取消,最长 30 分钟 |
