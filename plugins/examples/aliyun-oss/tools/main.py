"""阿里云 OSS —— 把素材传上去、换一条公网直链,以及把桶里的东西拉回素材库。
...
def upload(payload: dict, locale: str) -> dict:
    ...
    return {
        ...
        "summary": line(
            locale,
            f"已传到 oss://{bucket.bucket}/{key},直链 {expires} 秒内有效",
            f"Uploaded to oss://{bucket.bucket}/{key}; the link is valid for {expires}s",
        ),
    }
...
if __name__ == "__main__":
    storage.run({"oss_upload": upload, "oss_presign": presign, "oss_fetch": fetch, "oss_list": listing})
