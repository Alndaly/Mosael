...
ipcRenderer.on("mosael:fullscreen", (_event, value) => {
  lastFullscreen = Boolean(value);
});

// 通知点击 → 主进程要求打开任务中心。TaskCenter 监听的是 window 事件,这里做转发。
ipcRenderer.on("mosael:open-tasks", () => {
...
ipcRenderer.on("mosael:deep-link", (_event, link) => {
  window.dispatchEvent(new CustomEvent("mosael:deep-link", { detail: link }));
});
ipcRenderer.on("mosael:open-files", (_event, paths) => {
...
contextBridge.exposeInMainWorld("mosaelDesktop", {
  ...
  setTitleOverlay: (colors) => ipcRenderer.send("mosael:title-overlay", colors),
  ...
  onUpdateAvailable: (callback) => {
    ...
    ipcRenderer.on("mosael:update-available", listener);
    return () => ipcRenderer.removeListener("mosael:update-available", listener);
  },
  ...
  customCss: {
    ...
    onChange: (callback) => {
      ...
      ipcRenderer.on("mosael:custom-css", listener);
      return () => ipcRenderer.removeListener("mosael:custom-css", listener);
    },
  },
  onFullscreen: (callback) => {
    callback(lastFullscreen);
    const listener = (_event, value) => callback(value);
    ipcRenderer.on("mosael:fullscreen", listener);
    return () => ipcRenderer.removeListener("mosael:fullscreen", listener);
  },
});
