...
- **把内容变成新内容的工具住在格子上了,画板上不再有单独的工具格。** 选中一段音频,它上方的操作条上就是一排图标:转写、
  分离人声与背景音、降噪(和「剪一段」并排);视频还有「视频转 GIF」,便签和文档有「翻译」;插件工具按它吃什么内容挂在
  对应的格子上(吃一张图的 ComfyUI 工作流在图片格上)。内置的直接摆出来,多了收进「⋯」。点一枚,它的面板挂在格子下面;
  点另一枚,下面那块换成它的;再点一次收起。面板最上面写着对哪一格做什么 —— 这一格的内容就是输入,不用再挑、不用再连;
  底栏是它的设置芯片(写的是值,还差必填时发送键是灰的并说差什么),其余在「参数」里。每一项的设置存在那一格上,下次打开
  还是上次的样子;结果照旧新建在右边一列、连上线,这一格自己不动;在跑时格子底边一条运行态,能停。按提示词出图、出动画的
  插件工具(ComfyUI 文生图、Manim / Remotion)是空格子的一种填法,和「配音 / 生成」在同一个切换里。「添加」只剩格子,按
  生成 / 从素材库 / 引用 / 整理分组;拉线菜单也只列格子。智能体在格子上写能力的设置(`set_form` 带上那一项)、替你点运行
  (`run_board_item` 带上那一项),不再放工具格。升级时已有的工具格搬到它接着的那一格上(设置存进去、跑出来的产出留着并
  改从那一格连出),按参数出素材的变成那种素材的空格子,找不到能接的格子的变成一张写明原来设置的便签
  (迁移 `migrate-board-tool-cells-become-abilities`)。

#### 修好的

- **格子上跑完一项能力,通知里写得出是哪一项。** 此前翻译、转写这类跑完弹的是「画板工具 · 已完成 「」跑完了」——
  名字在排队时写上了,开始、跑完两句整句重写时没带上。
- **ComfyUI 挑了一个不带文本编码器的模型时,说清楚是哪个文件、该怎么办。** 此前是一段 ComfyUI 的英文原话
  (`CLIPTextEncode: ERROR: clip input is invalid: None…`)。Anima、Flux 这类模型的文本编码器(或 VAE)是单独的
  文件,放在 checkpoints 里用普通加载节点读不出来;现在点名是哪个模型文件,并说换一个完整的 checkpoint,或在
  ComfyUI 里存一张单独加载文本编码器的工作流再选它。(ComfyUI 插件 1.5.3)
- **画板上生成中的那一格不再是一块发亮的灰板。** 铺满整张卡的加载占位用更轻的一档底色和扫光,和周围的格子
  是一个亮度;列表、弹窗里的小块占位不变。
- **选择笔记、选择 3D 场景、挑素材是同一个弹窗,一行一项。** 此前笔记和场景一条占一大块(大图标、两行正文原文、
  再一行日期),挑到第四条就得滚;正文是直接截的 Markdown,一行里露出代码块围栏、图片地址和表格竖线。现在搜索钉在
  头里,每行是名字 + 一行去掉记号的摘要,版本和「多久前改过」靠右;打开就能打字搜,↑↓ 换行、回车挑。
  笔记列表的摘要用同一份去记号的规则。
- **悬停说明不再挂在上一枚按钮上。** 关掉菜单或面板时焦点还回那枚按钮,它的说明就一直挂着、移到别的按钮也不走;
  现在只在指针停在上面或用键盘切过来时出,同一时刻只有一条。
- **画板操作条:图标悬停马上说它是什么,「生成」收成一个菜单,不再被切出画布。** 能力图标、改名、复制、删除悬停
  300ms 就出说明(名字一行、那一句说明一行),不再是要停一秒多的系统提示。「生成 图片 视频 文案 音频」一排字收成
  「＋ 生成 ▾」,菜单里每一行把这一下说全(「用这张图当首帧生成视频」);便签上不再有「生成文案」—— 改写、翻译
  是它自己的能力。格子贴着侧栏时,操作条会平移回画布里(此前只在打开画板时就选着一格才生效)。
- **插件工具只有一个连接时,「参数」里不再摆「连接」。** 留空就是用那一个;有两个以上才让挑。
- **选项多的下拉在「参数」弹层和设置表单里撑满它那一格。** 此前选项超过 8 个时(比如项目多了),下拉掉到标签下面一行、
  只有标签那一列宽,右边空着一大块:可搜索的下拉在触发器旁边多放了一个隐藏的探测元素,表单行的排版把它又显示出来,
  占掉了下拉那一格。现在下拉只交出触发器本身。
...
- **画板上的 3D 场景格自己会渲白模参考,不再旁边另放一格「渲染白模参考」工具格。** 此前场景格只放缩略图、提示
  「打开场景，选择构图并导出参考画面」,要渲首尾帧还得再放一格工具格、在上面把场景重挑一遍 —— 同一件事的两半。现在选中
  场景格,面板底栏挑「镜头」(只有一个镜头时显示的就是它,没有镜头时是灰的并说明为什么)和「渲染内容」(首尾静帧 /
  运镜视频 / 都要),点圆键就渲;渲的时候格子上是和别的格子一样的扫光、进度和「停止」,首帧、尾帧、运镜视频新建在场景格
  右边并连上线,场景格和缩略图不动。「编辑场景」挪到面板上。跑的是工作流里那个节点的同一个渲染器、同一种任务(能停、
  记在同一处);工作流里的「渲染白模参考」节点不变。智能体也能在场景格上写镜头(`set_form`)、替你点渲染
  (`run_board_item`,本机渲染不出卡)。升级时画板上已有的这种工具格:接着(或填的是)板上某一格场景格的,镜头和渲染内容
  搬到那一格上、工具格撤掉,它渲出来的图和视频留着、改从场景格连出;找不到场景格的变成一张写明原来设置的便签
  (迁移 `migrate-board-scene-cells-render-themselves`)。
