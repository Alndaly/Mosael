...
- **ComfyUI 的每张工作流都是一个工具,参数不再是写死的。** 工具表、工作流节点面板和智能体里出现「工作流 · 名字」,
  表单就是那张图自己的:它的提示词、它读素材的节点(只列那一种素材)、它能调的参数(人话名字、常用在前、细节收进
  「高级」),输出按它的输出节点分。在 ComfyUI 里改名不影响;以前用「运行工作流」选了某张图的节点自动改成那张图的工具。
  插件页上工具的「试一下」和工作流节点用同一张表单:素材选择器、下拉、数字框,一串图是挑出来的一排,不再是 `[]` 文本框。
- **插件可以在运行时报出工具**(`provides: ["tools"]`):和清单里声明的工具一样进工具表、节点面板和智能体,
  目录变了自动刷新;报出的工具能声明它取代了哪个老工具,存着的老节点自动改写过去。写法见 `docs/PLUGIN_MANIFEST.md`。
...
- **画板助手会用工具格了。** 它能列出你能用的工具(插件工具只有你自己接的)、放一个工具格并填好字段、
...
- **ComfyUI 插件有了一组工具,智能体和工作流能直接用 ComfyUI。** 「运行工作流」原样跑任意一张工作流:图、
  蒙版、视频、音频接到读素材的节点上,任意节点的值能改,**全部产出**(每个保存节点的每一个文件、显示文字的节点
  说的话)进素材库 —— 放大、抠图、局部重绘、修脸、补帧用的都是你自己的图;跑得久的可以只提交,之后「导入 ComfyUI
  产出」按任务号取回(在 ComfyUI 界面里跑出来的也行)。另有「列出工作流」(每张收什么、能调什么、在做什么)、
  「服务器状态」(显卡、空闲显存、队列)、「列出模型文件」、「中断任务」,以及默认不开的「清空队列」「释放显存」。
