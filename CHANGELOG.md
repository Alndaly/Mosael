...
- **插件的「去授权」回到连接上,看得出授权过没有。** 此前它是凭据最后一行里的一颗按钮,排在 Access Token 下面,
  读起来像那一格的附属操作,也看不出这个连接到底授权了没有。现在声明了 OAuth 的插件(百度网盘),每个连接卡片抬头下面
  有一条「授权」:未授权 / 已授权 / 需要重新授权,按钮跟着写「去授权」或「重新授权」,授权码就贴在同一条里;
  授权会填的 Refresh Token、Access Token 收成一行只说填没填,点「手动填写」照样能自己填。令牌被对方作废时,插件在
