...
- **画板上的工具格一眼看得出是什么、还差什么。** 每个工具用自己的图标(翻译是「文A」、转写是话筒、分离人声是
  剪刀……,和工作流画布上同一颗;插件工具按它处理的内容),格子上方那一行和「添加」菜单里也是这一颗,不再个个
  顶着同一把扳手。空着的格子不再是一句说明加一大块空白,而是一份安静的摘要:吃什么(「文本 · 接便签或文档」,
  连上了就写那一格的名字)、必填或改过的一两个设置(「目标语言 · 英语」,没选写「待选」)、产出什么。内置工具
  不再挂一个「内置」小标签;插件工具在说明前点名插件,和「添加」菜单里的写法一样。
