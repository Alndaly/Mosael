...
- **把内容变成新内容的工具住在格子上了,画板上不再有单独的工具格。** 选中一段音频,它上方的操作条上就是一排图标:
  转写、分离人声与背景音、降噪(和「剪一段」并排);视频还有「视频转 GIF」,便签和文档有「翻译」;插件工具按它吃什么
  内容挂在对应的格子上(吃一张图的 ComfyUI 工作流在图片格上)。内置的直接摆出来,多了收进「⋯」;每一枚悬停马上说它
  是什么。点一枚,它的面板挂在格子下面;点另一枚,下面那块换成它的;再点一次收起。面板最上面写着对哪一格做什么 ——
  这一格的内容就是输入,不用再挑、不用再连;底栏是它的设置芯片,其余在「参数」里。每一项的设置存在那一格上,下次打开
  还是上次的样子;结果新建在右边一列、连上线,这一格自己不动;在跑时格子底边一条运行态,能停。按提示词出图、出动画的
  插件工具(ComfyUI 文生图、Manim / Remotion)是空格子的一种填法,和「配音 / 生成」在同一个切换里。「添加」只剩格子,
  按生成 / 从素材库 / 引用 / 整理分组,每一行一个名字加一句说明;拉线菜单也只列格子。智能体在格子上写能力的设置
  (`set_form` 带上那一项)、替你点运行(`run_board_item` 带上那一项)。升级时已有的工具格搬到它接着的那一格上(设置存
  进去、跑出来的产出留着并改从那一格连出),按参数出素材的变成那种素材的空格子,找不到能接的格子的变成一张写明原来
  设置的便签(迁移 `migrate-board-tool-cells-become-abilities`)。
- **3D 场景格自己会渲白模参考。** 选中场景格,面板底栏挑「镜头」(只有一个镜头时就是它)和「渲染内容」(首尾静帧 /
  运镜视频 / 都要),点圆键就渲;首帧、尾帧、运镜视频新建在右边并连上线,场景格不动,「编辑场景」挪到面板上。跑的是
  工作流「渲染白模参考」节点的同一个渲染器、同一种任务(能停、记在同一处)。升级时原来那种「渲染白模参考」工具格的设置
  搬到它接着的场景格上(迁移 `migrate-board-scene-cells-render-themselves`)。
- **「接着做」收成一个「＋ 生成 ▾」菜单。** 每一行把这一下说全(「用这张图当首帧生成视频」「把这段文字念成音频」),
  图标是要长出来的那种格子;便签上不再有「生成文案」—— 改写、翻译是它自己的能力。
- **音频文件拖进画板会放一格音频,画布上能 ⌘V 粘贴:** 截图和复制的媒体文件各放一格,一段文字落成一张便签;光标在
  便签、提示词框里时照常贴进那里。
- **选择笔记、选择 3D 场景、挑素材是同一个弹窗,一行一项。** 搜索钉在头里,每行是名字 + 一行去掉 Markdown 记号的
  摘要(不再露出代码块围栏、图片地址和表格竖线),版本和「多久前改过」靠右;打开就能打字搜,↑↓ 换行、回车挑。
- **3D 场景记得你上次从哪儿看。** 自由视角的相机位置、环绕中心和视场,「俯瞰全场」里拖过的取景,当前视角档和镜头都按
  场景记在本机;不改场景、不涨版本、不同步给别人。
- **出图 / 视频的下拉里只有真能出图 / 视频的模型。** 生成能力要有依据:内置目录认得、插件声明过、你写过它的参数、或者
  这家只做这一件事;其余只当对话模型(此前画板的出图格里列着 `claude-opus-4-6`)。能出图却没被认出来的,在「设置 →
  模型列表」点它的能力标签标上即可。升级时你设成默认的、真出过图 / 视频的模型,那项能力照旧保留。生成选择器预选
  「设置 → 默认模型」里那一个,没设时显示「选择模型」。

#### 修好的

- **音频格生成中的占位贴满格子。** 此前占位两边各缩进一截(给播放器留的左右留白也套在了占位外面),看着像一块小一号
  的卡叠在格子里。
...
- **在跑的格子不再有溢出格子的光晕;格子四角和里面图片的圆角对齐了。** 运行态只画在格子自己的边框上;所有格子一个
  外壳圆角,贴着内沿的图、视频、占位和它同心,角上不再露一道暗缝。
- **画板面板一个样子,发送键不再被挤出面板。** 生成、写字、配音、剪一段、跑一项能力:上面是上游芯片,正文自己滚,
  底栏钉在面板底部 —— 设置芯片(写的是值,「英语」「镜头 1」;还没选时淡淡写字段名)+「参数」+ 一枚圆形发送键。
  必填的没填时发送键是灰的、悬停说还差哪几样;正文的占位是一句字段说明;「参数」是一张表、一种排法。插件只有一个
  连接时不再摆「连接」。
- **格子贴着画布边时,面板和操作条平移回画布里,** 不再钻到侧栏底下;下方放不下时面板收矮,发送键仍钉在底边。
- **选项多的下拉在「参数」弹层和设置表单里撑满它那一格。** 此前选项超过 8 个时下拉掉到标签下面一行、只有标签那一列宽:
  可搜索的下拉在旁边多放了一个隐藏的探测元素,被表单行的排版显示了出来。
- **可搜索下拉里同名的几项不再一起高亮、选错。** 列表按每一项自己的 id 认行;同名的场景、项目等在名字后面带上一截 id。
- **悬停说明不再挂在上一枚按钮上。** 关掉菜单或面板时焦点还回那枚按钮,它的说明就一直挂着;现在只在指针停在上面或
  用键盘切过来时出,同一时刻只有一条。
- **格子上跑完一项能力,通知里写得出是哪一项,** 不再是「画板工具 · 已完成 「」跑完了」。
- **每一种在跑的格子都能停。** 生成、配音、写字、剪一段在跑时也有「停止」;远端生成停下之后供应商可能照样计费,按钮上说明。
  生成完不再留一圈绿框;跑挂了的格子说「生成失败」/「运行失败」和原因,不再一律写「没能发起生成」。
- **画板上生成中的那一格不再是一块发亮的灰板。** 铺满整张卡的加载占位用更轻的一档底色和扫光。
- **选中便签不再弹出写作面板。** 挪一挪便签不用先关面板;要 AI 帮忙时点操作条上的「让 AI 写」。
- **画板上几处对不齐的地方。** 配音面板「还没有克隆音色」的说明挪进正文;打开面板时缩略图淡出,不再压在发送键上;
  还没选笔记的文档格不再重复写「文档」;选中的文档不再套一圈主色描边;英文界面里 3D 场景格叫 "3D scene"。
- **ComfyUI 挑了一个不带文本编码器的模型时,说清楚是哪个文件、该怎么办。** 此前是一段英文原话(`clip input is invalid:
  None…`)。Anima、Flux 这类模型的文本编码器(或 VAE)是单独的文件;现在点名是哪个模型文件,并说换一个完整的
  checkpoint,或在 ComfyUI 里存一张单独加载文本编码器的工作流再选它。
- **ComfyUI 工作流在画板上只有一个入口、只落它的产出。** 只有一个图片 / 视频 / 音频输出节点的工作流就是那种格子里的
  一个模型(看线稿的 PreviewImage 这类预览不算输出节点),右边不再多出同一张图、素材 id 的 JSON、摘要和任务号;只交出
  一段字的工作流(打标签、反推提示词)不再冒充图片模型。插件用 `mirrors`、`wiring_outputs` 声明这些关系。
- **画板上不再出现按编号去外面取东西的工具。** 「导入 ComfyUI 产出」「从百度网盘导入」「从对象存储取回」要的是另一个
  系统里的任务号、fs_id、对象路径,创作者在画板上填不出来;插件在字段上声明 `format: "external_id"`,这种工具只在
  工作流和对话里用。
- **插件工具的名字不再拿说明顶替。** 名字依次取清单里写的名字 → `node` 块的名字 → 按调用名人性化;随附和示例插件的
  每个工具都写了中英两份名字。
- **指向某样东西的字段是下拉了,不用再去别处抄 id。** 3D 场景和镜头(镜头跟着场景走)、时间线、轨道、项目、插件连接;
  工作流里这些下拉也列上游的输出、能写 `{{…}}` 引用。
- **画板上一格挂的素材要在这个工作区里、种类对得上格子。** 一段音频不能再被放进图片格;报错说清是哪一格、哪份素材。
- **3D 场景「拿去生成」建的画板,生成那一格选中就有提示词面板。** 升级时已经存着的这种格子自动补上。
- **「设置 → 本机引擎」里的模型和引擎列表回到设置页的样子:** 每个引擎是分组里的一行,下载进度是横贯整行的一条细进度条。
- **智能体的 `edit_board` 说明补全了:** 3D 场景格必须带 `scene_id`,放已有素材时种类要对得上。

插件:ComfyUI 1.5.3、百度网盘 0.7.2、对象存储 1.0.2、Blender MCP 0.3.2、Text Toolkit 0.2.1。
