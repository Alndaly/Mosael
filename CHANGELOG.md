...
- **画板上能跑工具了。** 添加菜单和拉线菜单里多了「工具」一组(分组和工作流的节点面板一样,可搜):插件工具
  (只列你自己接的)和挑过的工作流节点都能摆成一个工具格,字段照节点表单填,上游的便签、文档、素材、场景连上线
  就是输入;点运行后产出在工具格右边新建一列并连好线,重跑不覆盖。运行中有扫光、进度和停止,失败说原因;空的
  音频槽可以在「配音」和「生成」之间切。
