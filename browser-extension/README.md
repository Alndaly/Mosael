...
- Accept video URLs recognized by the installed yt-dlp build; YouTube and Bilibili prefer native captions, while Open Studio can download and transcribe other sites.
...
- Choose a destination workspace, optional project, and optional Open Studio Browser Pool identity.
...
4. The first time you auto-transcribe, use AI translation, import, or capture, open settings in the panel, sign in with your
   Open Studio account, and choose a destination workspace. For member-only, private, region-restricted, or bot-protected
   media, optionally choose an identity already signed in and configured with a proxy in Open Studio's Browser Pool.
...
The panel can select an identity already managed by Open Studio and sends only its ID to the backend;
