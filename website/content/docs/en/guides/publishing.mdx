...
![Publish page: publish records and account matrix tabs](/media/screens/publish.png)
...
![Demo: add-account dialog in the account matrix and the new-publish dialog](/media/gifs/publishing.gif)
...
Logins run in an embedded browser view with an address bar / back / forward / reload and a "Back to Mosael" button.
...
Once a platform is picked the form fills in that platform's **own options**: visibility (private /
unlisted / public), YouTube's "made for kids", RedNote's originality declaration. They are declared in
one place and the form renders from the declaration — **Bilibili and WeChat Channels were checked and
their publish pages have no visibility control, so the field does not appear there**. A switch that
does nothing when you flip it is worse than no switch.
