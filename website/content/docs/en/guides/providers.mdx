...
Mosael's AI features (agent chat, text-to-image / video, voiceover, transcription) are bring-your-own-key: most vendors take an **API key**, subscription plans use **authorized sign-in** (below). Credentials never leave this machine; the settings API returns a masked hint or just "linked / not linked", never the plaintext.
