...
to their creator; Open Studio never lists, selects, or falls back to another user's connection.
...
interchangeable. Open Studio groups them under one company namespace while retaining separate
...
Hailuo, WAN, Sora, GPT Image, Gemini and Seedream model rows under the same connection. Open Studio
...
different regional, account or content-policy behaviour. Open Studio adds no local "real person"
