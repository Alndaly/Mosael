...
**画板上只放内容变换**(`boards/transforms`,ADR 0021 修订):吃画板上的素材 / 文字 / 3D 场景或凭空产出
素材,交出素材或点名落板的文字,不是流程 / 数据 / 知识库节点,必填字段创作者填得了;流程控制和数据搬运
归工作流。
插件工具用**点运行的人自己的连接**。智能体也能放工具格、改它的表单(`edit_board`)、替人点运行
(`run_board_item`):只读的直接跑,花钱或对外的出确认卡,用批准者自己的连接。
_Avoid_: 按 item 种类猜该挂哪块面板;为某一种产出单开路由或请求体;把产出者当成 item 种类;
在画板那边另列一张「能上画板的节点」清单(声明在 NODE_TYPES 上);把流程控制、数据搬运、调用工作流放上画板
(画板是「想法摊开,边看边做」,工作流是「做一次,跑无数次」)
