...
产出者 `node:<节点类型>` 跑一个工作流节点 —— 这个人自己接的插件工具,或声明了 `surfaces: [..., "board"]` 的
内置节点。**画板上只放内容变换**(`boards/transforms`,ADR 0021 修订):吃画板上的素材 / 文字 / 3D 场景或凭空产出
素材,交出素材或点名落板的文字,不是流程 / 数据 / 知识库节点,必填字段创作者填得了;流程控制和数据搬运归工作流。
插件工具用**点运行的人自己的连接**。

**能力(Ability)**:
内容格自己会做的事(ADR 0025 修订「能力住在内容格上」)—— 音频 / 视频格会转写、分离人声、降噪,视频格会转 GIF,
便签 / 文档会翻译,插件工具按它吃什么内容挂在那几种格子上(`role: "ability"`,从字段声明推)。**宿主那一格的内容
就是它的输入**(`host_fields`),设置按产出者存在宿主的 `form.abilities` 上,这一轮是哪一项记在 `run.ability`,产出
新建成宿主右边的一列、连线。选中一格,操作条上一排图标就是它的能力;点一项,它的面板挂在格子下面。凭空按参数出素材
的插件工具不是能力,是**空格子的一种填法**(`role: "slot"`,和配音 / 生成一起在空槽的切换里)。智能体用
`set_form`(带 producer)在宿主上写设置、`run_board_item`(带 producer)替人点运行:只读的直接跑,花钱或对外的
出确认卡,用批准者自己的连接。
_Avoid_: 按 item 种类猜该挂哪块面板;为某一种产出单开路由或请求体;把产出者当成 item 种类;
在画板那边另列一张「能上画板的节点」清单(声明在 NODE_TYPES 上);把流程控制、数据搬运、调用工作流放上画板
(画板是「想法摊开,边看边做」,工作流是「做一次,跑无数次」);再放一格单独的「工具格」去处理另一格的内容
(同一件事的两半 —— 它是那一格的能力)
