...
运行期双读垫片已经收敛:自有数据在启动/首次读取时迁移并删除旧来源；用户持有的工作流导出文件
和旧环境变量只在输入边界归一。Electron 壳、renderer、后端与 worker 必须来自同一 Release，
不再用旧协议名或旧报文头猜测混装版本。
...
格式支持的修改具有 Locality。前端凡是把图片交给 `<img>`、Canvas 或大图灯箱，都必须使用
`assetPreviewUrl`；`assetFileUrl` 只用于下载原件以及视频/音频播放。不能因为缩略图已经是 JPEG 就在点击
放大时切回原文件——HEIC 会在缩略图正常的同时于详情页破图。详情弹窗 DOM 回归钉住内联与放大两条路径，
画布回归测试钉住 HEIC 的真实 JPEG 字节和 MIME，以及 WebP 原字节与真实 MIME；素材分析已有独立 HEIC
回归。这里不能只检查“系统里有 ffmpeg”：Linux 发行版常裁掉 HEIC demuxer，二进制存在不代表具备该
能力。回归还会禁用 ffmpeg 路径，确保安装包使用自带解码器。图片数量、编码大小和视频输入仍由各传输
Adapter 按自身协议限制，不强行塞进格式归一化 Module。
...
## 11. 全仓架构审查(2026-09-18)

一次按「依赖方向 / 重复实现 / 巨型模块」扫过全仓的审查。十处已整改,记在这里是因为**每一条的
形状都会复发**,下次加功能时值得先对一眼:

- **规则跟着"事情"走,不跟着入口走。** 字幕配音的「原声怎么办」曾只长在工作流执行器里,于是
  剪辑台和智能体发起的配音选不了;「没点名就用默认模型」在三个入口各抄一份;定时任务的执行分派
  长在轮询循环里,而「立即运行」和 webhook 各拼各的(其中一条忘了查重入)。判据:**同一件事有
  第二个入口时,规则必须先收敛,再接入口。**
- **路由不是第二个领域层。** 画板的四个动作(生成/写字/念/截)整个写在路由里,连提示词拼装和
  素材摊平都在。现在在 `domain/boards/actions.py`,路由只认人、翻错误。后来(ADR 0021)四个动作
  又收成一张产出者注册表(`domain/boards/producers.py`),四条路由合成一条 `/run`。
- **私有名字不跨包借用**(`tests/test_private_names_stay_in_their_package.py`):被借的那一刻它就是
  一个没人承认的接口 —— 而四处里有三处借的其实是**放错了地方**的规则。
- **抄第二份就会漏掉修复。** 下载进度/就绪探测曾有三份实现,其中"探测代次"那个坑只修在一份上。
- **一个东西的知识摊在四处**:确认卡的每个工具曾要在权限表、校验链、摘要链、执行链各补一段;
  工作流节点的「后果落在哪」曾是与声明并列的两张表 —— 而插件节点(运行时才知道类型)就这么漏了。
  现在都是**声明的一格**。
- **前端底层不认识功能模块**(`design/layering.test.ts`);**接口路径只写在 `api/domains/*`**
  (`design/apiSeam.test.ts`,存量只减不增)。
