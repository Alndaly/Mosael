...
拖动、逐字编辑、失败/取消不会清空，输入必须留给重试。同步便签写作虽不创建后台 job，也必须在节点
内经历 running → succeeded/failed，节点外壳与按钮读取同一生命周期。
...
| `options_from` | 选项要现查:来源名 | 清单由 `domain/workflows/field_options.SOURCES` 里同名的函数给;有 `depends_on` 时,父字段的值作为 `parent` 带上,清单跟着它变。编辑器对所有这种字段走同一个接口(`GET /api/workflows/field-options`)。**一对互相牵连的字段(引擎 + 音色)是两格声明,不是编辑器里的特例** —— 语音节点曾把音色存成两个键、由编辑器按引擎显示其一,两个键一前一后,换引擎时那一格上下跳,看起来就是两个音色框。加一个来源 = 加一个函数;棘轮钉着"声明里用到的来源都存在" |
...
3D 的 `scene_types` 定义物体、相机和关键帧；镜头通过 `camera_id` 引用物体。`sceneGraph` 与 `sceneTracks` 处理姿态采样、分组和帧编辑，`SceneDopeSheet` 只负责时间线交互。参考帧、灰模与 GLB 共用去除编辑辅助对象的导出副本。模型使用数据目录中的文件存储，数据库保存归属与版本。
