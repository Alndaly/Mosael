...
| `sole_option_default` | 留空 = 清单里只有一项时用那一项 | **是默认,不是预选**:表单把那唯一的一项显示成当前值,但不替人写进配置 —— 父字段常常是上游接进来的(工作流的数据边、画板的场景格),编辑那一刻清单还不知道;做决定的是运行时的同一条规矩(插件连接的 `resolve_instance`、渲白模的镜头),有好几项时不猜、报出来 |
| `media` | 素材字段只收哪几种素材 | 一种写字符串(`"video"`),几种写列表(`["audio", "video"]`)。素材选择器只列这几种,画板上只有这几种格子接得上(`boards.tools.bindable_kinds`,写绑定、运行时取值认同一份)。转写没声明时,画板格子上写着「接图片、视频或音频」 |

**指向某样东西的字段给选择器,不给文本框**(棘轮 `test_entity_fields_have_a_picker`):字段名是
`<实体>_id(s)`(场景、镜头、项目、时间线、轨道、账号、连接、工作流……)或推导出的数据类型是
asset / sequence / scene 的,声明里必须说清单从哪来 —— `options_from`、`options`、`editor`,或是素材字段。
挑出来的值在工作流里仍然可以是一段 `{{…}}` 引用:工作流的检查器告诉表单「这里能写引用」(`references`),
下拉在这个工作区的东西后面列上游的输出,手敲只收引用(随手敲的一串字是一个不存在的 id);画板和插件的
「试一下」里值只能是清单里的一项,给标准下拉(项多了自动换成能搜的那一版)。
