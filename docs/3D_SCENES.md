...
`blender_look` 不只有六个预设机位:视角可以写成 `"<方位角>/<仰角>"`(度,仰角 ±89 以内 ——
正上方时"哪边是上"没有定义,画面会莫名其妙地转),`zoom` 0.2~8 决定凑多近,`objects` 只框住
点名的那几个。**细节看不看得见,取决于这三样**:整场景取景下一处接缝只有几个像素,而那时它和
"没建出来"长得一模一样。

着色三档:`solid`(快)、`xray`(看穿 —— 两件东西是不是真的穿在一起、内部有没有多余几何)、
`rendered`(EEVEE,看真材质和灯光)。**没有线框**:Workbench 的线框只在视口里有,F12 渲出来是
一张空背景(实测 Blender 5.2,设了 `wireframe_color_type` 也一样)。
