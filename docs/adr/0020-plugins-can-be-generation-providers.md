...
## 补充(2026-09-26):通用的 `run_workflow` 删掉

第三句反馈:「它连要跑哪张工作流都不知道,却要人填参数;不同工作流的参数本来就不一样。」留着当「智能体的退路」的
`run_workflow` 在插件页上照样有一张「试一下」表单(workflow / prompt / image / images / mask / video / audio /
values + 8 个高级项),开着它的连接在节点面板、画板工具格和智能体工具表里也照样有它 —— 表单从构造上就是错的。
逐条对过它比每张图自己的工具多出来的本事,没有一条站得住:

- `builtin:txt2img`:它本来就是一个生成模型;为了让存着的节点有去处,也报成一个工具 `wf_builtin_txt2img`;
- `api-workflow`:早就有 `wf_api_template`;
- `values` 改任意节点:可调的字面量输入都已经是工具的一格;列不进来的是文件名前缀这类不在 Mosael 里调的,
  和 object_info 里说不出类型的 —— 要改就在 ComfyUI 里改那张图;
- 刚存、清单还没刷新的图:目录巡检一分钟内刷新,插件页有「刷新」;
- `wait: false` 只提交:工作流工具标的是 `paid`,智能体调它是开卡、批准后在后台跑,不占它一次调用的 3 分钟;
  更长的活本来就该走生成(6 小时、回执、续等);「导入 ComfyUI 产出」照旧能按任务号取回。

**决定:删掉,不做成「只给智能体」。** 没有一个真实需要,就不为它发明一个「人看不见、智能体看得见」的清单字段
(`internal` 是「只给宿主」,智能体也调不了,本来也不合适)。存着的 `run_workflow` 节点照旧由每张图的工具声明
`replaces`、对账步骤 `rewrite-replaced-plugin-tools` 改写 —— 插件要一直报这份对应关系,因为它依赖 ComfyUI 那边的图。
为此宿主的改写多了一条**通用**规矩:**老工具在那个连接上已经不在了**时,对不上的格子、数据边、画板绑定丢掉,
丢了什么写进那一版修订的说明,而不是留下一个注定跑不起来的节点(老工具还在时照旧不改)。插件的 `rename` 为此
写全:`values` 按节点 id 和按节点标题的两种写法、种子和尺寸的那几格都有去处。只挂着工具名的开关和会话里
「本会话始终允许」由一次性迁移 `forget-comfyui-run-workflow-tool` 清掉;选的那张图已经删了的节点无从改起,留着,
和指向已删工作流的 `wf_…` 节点同一种状态。
