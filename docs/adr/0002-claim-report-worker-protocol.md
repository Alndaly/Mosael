...
## 浏览器自动化通道(`/api/browser/worker/*`)

粒度是**单个动作**(`BrowserAction`)而不是一个任务:调用方 `run_action` 入队一条、阻塞轮询到
终态。除此之外它和通用通道**同一份契约**:认领带 `lease_token` + `lease_expires_at`(同样 60 秒)、
回报原样带回、心跳带 `{"worker":…, "claims":[{"action_id":…, "lease_token":…}]}` 并返回 `renewed`,
没续上的执行应停止;租约到点的动作判失败,可重试。

**这一段是补写的。** 这条通道存在了很久而 ADR 里一个字都没有,实现上也一条都没落:
`ClaimRequest.worker` 收下了却一次都没用过、表上没有租约列、客户端发的是字面量 `"browser"`
而不是身份、心跳只说"我还在"(那个 `_HEARTBEATS` 字典全仓零个读者)。没有可见现象 ——
单执行器下它工作正常,问题是**没有任何东西能在多执行器或执行器崩溃时保证正确**。

一条 ADR 说"任何跨进程执行都走同一个契约",而第三条通道整条不在它的视野里 —— 这比通道本身
的缺陷更值得记:**写下一条普遍规则不会让它自动适用于下一个实现**。
