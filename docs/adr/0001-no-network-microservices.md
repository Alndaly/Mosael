...
Mosael 是本地优先桌面应用:单机上把后端拆成多个 HTTP 服务只会引入序列化、版本兼容、部分失败与分布式取消,而收益(独立伸缩/部署)在单机上不存在;SQLite 单写者也决定了数据无法按服务切开。剪辑内核(SequenceOperation + revision + 撤销)必须保持单事务原子性,永远不跨进程拆。
