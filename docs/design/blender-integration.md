...
- 发送用的 GLB **由后端生成**（`app/domain/scene_render/gltf.py`，与白模渲染器同一份几何）：物体、分组父子关系、PBR 材质与点光源都在里面，镜头由 worker 按 `shots` 另建为真正的 Blender 相机，导入的模型各自作为文件导入再挂到锚点节点下。此前这一份是浏览器用 three.js 导出上传的，于是发送场景必须有人开着那个页面——跑在后端的智能体做不到。现在界面按钮和 `blender_send_scene` 是同一条路。
- [`模型导入`](../../backend/app/domain/scenes.py) 接受自包含 GLB/内嵌 glTF；当前限 512 MB（内嵌 glTF 100 MB）、5000 个节点、2000 个网格。复杂 Blender 场景需要生成预览版本，并提供明确的超限反馈。
...
- 使用固定版本 Blender MCP(上游 2.0 起改名 `mcp-for-blender`,当前钉 2.0.3)，通过项目已有插件实例、权限与工具调用路径连接独立 Blender 5.2.0 LTS 图形界面。
