...
**状态：调研方案，模型往返已实测；尚未实现或安装正式 Blender 插件。**
...
6. GLB 经现有场景模型 API 导回 Mosael，实际视图加载并显示成功，还能再次导出。当前回传是一个模型对象，未恢复成独立原生对象或原生 `SceneShot`。

测试产物：`.blend` 100886 字节，Blender GLB 10412 字节。结果证明基础模型链路可行；没有验证高质量渲染、实时连接、Windows、其他 Blender 版本、复杂材质或任意工程的无损往返。
