...
| ComfyUI(第一方插件) | 由用户工作流决定 | 每张保存的工作流是一个模型，插件从工作流里读出参数与素材槽：LoadImage 接到 `start_image` / `end_image` 的是首/尾帧，其余是参考图；视频输入(LoadVideo)尚未接 | 工作流自定义 | 本地实跑 + 模拟服务器测试 |
...
| P2 | ComfyUI 工作流的视频输入(LoadVideo / VHS LoadVideo) | 图片槽位已按工作流映射成角色(ADR 0020)；视频槽位要先定上传与帧数截取的语义 |
