...
> 状态:**已实施**(四期于 2026-08-03 全部落地;本文保留为设计与取舍的记录)。设计见 [ADR 0007](adr/0007-agent-permission-modes.md);本文是它的落地方案,
...
- **模式必须可见**。落地时按产品所有者的要求收进了「会话设置」弹层,同时在收起状态的设置按钮上
  挂一个随档变色的点 —— 非默认档在任何时候都看得见,而默认档不留常驻噪音。
...
```jsonc
{
  "http_request": "ask",    // ask | judge | always
  "publish": "ask",
  "run_code": "ask",        // 沙箱里跑(Docker,无网、看不到本机文件)
  "run_host_code": "ask",   // 不隔离,直接在用户电脑上跑(只在本机桌面版)
  "blender": "ask",         // 智能体在用户的 Blender 里执行建模代码(blender_execute)
  "notes": "自由文本准则,原样喂给判断者"
}
```

每一档都是确定性的,可测、可解释、事后能复算;自由文本只作为判断者的补充依据,不能单独放行任何
东西。两类代码执行默认都是 `ask`:「这段 Python 安不安全」没有可结构化的判据,让判断者去看一段
代码等于把闸交给最不可测的一环 —— 想开的人显式改档。沙箱与不隔离是**两个工具、两档准则**:
对「算个数」点过「本会话始终允许」或放开 `run_code`,都不会连带放开「动我的文件」。
Blender 建模同理自成一档:它也是本机代码,但一次建模常有几十步,放开它是建模时的常态,不该因此
连带放开 `run_host_code`。
...
| 3 | 三档本体:schema + 判定 + 后台执行 + 留痕 + UI + 「始终允许」迁移 | **已落地**(2026-08-03) |
| 4 | 准则(结构化 + 自由文本)+ `hold_until` + 隔离判断者 | **已落地**(2026-08-03) |
