...
| 层 | 张数 | 有哪些 |
| --- | --- | --- |
| **工作区级**(有 `workspace_id`) | 35 | 素材、序列、片段、项目、工作流、发布账号、浏览器档案、定时任务、智能体会话与记忆、确认卡、用量事件、通知、字体、LUT、声音… |
| **用户级**(有 `user_id`、无 `workspace_id`) | 2 | `AuthSession`、`OAuthIdentity` |
| **实例级**(两者都没有) | 23 | 供应商档案与密钥、模型、默认模型、网络配置、TTS 配置、AI 运行时配置、插件包/实例/凭据/授权、`User`、`Workspace`,以及若干挂在父表下的从属表 |
