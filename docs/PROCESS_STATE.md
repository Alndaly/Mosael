...
> **落在库里的「进行中」是另一回事,在另一处登记**:`app/domain/restart.py`。进程内的东西
> 重启就没了,而**在调用之前就落库的那些行不会** —— 它们停在最后一刻的样子,在界面上看
> 起来像还在跑。那张表由 `test_every_in_flight_row_has_someone_to_settle_it.py` 按 ORM
> **推导**着问:状态列默认成「进行中」的每一张表,要么给收尾函数,要么写清为什么不需要。
> (这条规矩此前被人工建立过四次,而第五、第六处照样漏了 —— 插件调用记录永远停在 running,
> Blender 互通记录永远停在 sending。)
...
**但在测试套里它们会串台**,而这份清单原先回答不了这件事 —— 它问的是"起第二个进程会怎样",
不是"一条测试改了它、后面的测试会怎样"。实测:一条测试把 `max_retries` PUT 成 6 之后,
同一条"连不上时目录为空"的测试从 4.68 秒涨到 26.37 秒(全量里 49.5 秒,因为还有别处推到了 9)。
**测试全绿,只是慢;而"慢"在三千多个点里是看不见的。**

登记做到了,还原没做到 —— **登记本身制造了一种已受控的错觉**。现在
