...
Mosael's workflow canvas is Dify/Coze-shaped: an edge means **execution order + branch
...
But pure ComfyUI is **not enough** for Mosael: sometimes there is no parameter to pass — you
...
3. **Per-output sockets** (ComfyUI-style) — one socket per declared output on the node's right
   side; drag from the specific output. Most Mosael nodes have 1–3 outputs, so the dots are cheap
