...
- 这一版改过、而截图没跟上的界面，按版本加在 [docs/media/RECAPTURE.md](media/RECAPTURE.md) 最上面 ——
  **写实际改过的那几处**，不要照抄上一版（照抄就是谎报）。这份清单只给维护者看：它此前写在官网
  「关于项目」页的正文里，每版往后接一句，九个版本下来成了一大段读者看不懂的流水账。官网那页只留一句
  「截图可能比应用旧」，发版时只改那句里的版本号。
