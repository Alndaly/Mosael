...
- 同一条路上还有两件事:同时在跑的插件调用受 `jobs.PLUGIN_SLOTS` 限额(排队前先交还数据库连接);
  ...
  停下的是整棵进程树(它起的 `node`、`ffmpeg` 一起停,见 core/child_process 的 own_group / kill_tree);
  stdout 读到上限(1MB)就停,不先整个读进内存。MCP 类插件做不到这一点 ——
  server 进程由 MCP 客户端库在事件循环里起,拿不到句柄 —— 取消后那次调用照常跑完,结果被丢掉。
...
另有 `MOSAEL_PLUGIN_DATA_DIR`:插件自己的持久目录,跨调用、跨更新都在,卸载时随包删(见 PLUGIN_MANIFEST 的「持久目录」)。
...
**一次几份**也是同一条路:`artifacts`(一串)换成 `assets` / `asset_ids`,素材数组(`items.format: asset`)换成
一串本地路径。**跑得久的工具**声明 `stream: true`,stdout 改成和生成同一套 NDJSON:进度经任务总线的上报口
(`jobs.report_progress`)成为工作流节点的进度事件,取消时先建取消文件让插件去停远端的活 —— 不流式的工具取消
就是杀进程,远端的活(ComfyUI 的一张图)会被它自己跑完。
...
- 实例一建,生成域就建一条 `provider_profiles` 行(`plugin_instance_id` 外键、级联删,厂商
  `plugin:<包 id>`);插件报来的模型清单缓存成 `provider_models` 行(`source = "plugin"`,
  每个模型自带的能力描述存在 `declared_capabilities`)。
- 注册表多一个**动态适配器来源**:按厂商前缀给出同一个 `PluginGenerationAdapter`,它把一次生成
  翻译成对那个工具的一次流式调用。于是 AI 工作台、画板、工作流「AI 生成素材」、智能体工具、
  生成历史、任务中心、回执、用量台账一行都不用认识插件。
- 能力描述的解析顺序变成:用户手写的 → **连接声明的** → 内置目录 → 兜底。宿主词汇以外的参数
  放在描述符的 `parameter_schema`(JSON Schema 子集),三个前端界面用同一个通用控件渲染,后端提交时按它校验。
- 选择器只列**自己的**插件实例建出来的连接(连接本来就按人归属);这种连接在设置页只读,改名、
  改地址、删都回插件页 —— 两边各改一半的话,下一次同步就把另一半冲掉了。

**长时间运行**:协议从「一问一答」扩成 NDJSON 事件流(进度、远端回执、最后一行结果)。取消是一个
文件 —— 宿主建 `MOSAEL_PLUGIN_CANCEL_FILE`,插件停远端的活后退出,30 秒不退就杀;这个开关就是进程在
任务名下登记的那个「子进程」,任务总线取消时拉的是它,不另起一套。生成调用不占 `PLUGIN_SLOTS`
(它已经在 `GENERATION_SLOTS` 里排过队了),预算按能力给(默认 1 小时,上限 6 小时)。回执落进
`Job.payload.remote_task`,后端重启后带着它再调一次 —— 和内置供应商的轮询循环(ADR 0019)是同一格、同一个判据。

**这条缝同样两头互不认识**:`domain/plugins` 不 import 生成域。它只有 `host_capabilities` 一张登记表
(能力名 → 「这个实例变了,你看着办」),实例的建 / 改 / 启停 / 授权 / 凭据变化都通知一声;
生成域在组装根把自己登记进去,收到通知去重问模型清单、对账那条供应商连接。插件那一侧的契约
(`domain/plugins/generation`:模型清单怎么规整、输入怎么拷进暂存、产出怎么取回)也不知道有 ProviderProfile 这回事。
...
**应用装的索引是发版产物,不是 main。** 默认地址是最新一次正式发版附带的 `registry.json`
(`releases/latest/download/registry.json`,`routes/plugins.DEFAULT_REGISTRY_URL`):发版流程打完插件包
之后,由同一个生成器(`scripts/sync-plugin-registry.py --release <tag> --packages <目录>`)从同一份
`plugins/examples/*` 生成,每条下载地址钉在那个 tag 上,生成时逐个打开包核对 id 与版本 —— 许的就是给的。
官网上那份(`website/public/plugins/registry.json`,不带参数跑同一个生成器,`"channel": "main"`)说的是
main 上的源码,新版本应用不从它装。此前应用读的正是它:main 上改了插件版本、还没发版时,它许 0.2.0、
...
市场对这一条不再说「有新版」。索引的许诺一变(发了新版,地址换到新 tag)、装上任何一版、或者过了
24 小时(兜自建索引一直用同一个地址的情况),这条就不算数。从链接装不带许诺,同一版重装照常。

第三条是**随应用发**:`plugins/bundled/` 下的插件(今天是 ComfyUI)跟着后端一起打包,每次启动由一个
常驻迁移步骤按内容指纹对账装进插件目录,和市场装的包落在同一处、走同一个 `packages.register`。它们卸不掉
(不用就停用连接),也不单独发版 —— 应用升级就是它们升级。

它们**照样在市场里搜得到**:用户不知道哪些是内置的,他会去市场里找「ComfyUI」,找不到就以为没有。
市场索引(`scripts/sync-plugin-registry.py`)同时收 `plugins/examples/` 和 `plugins/bundled/`,内置的
条目标 `bundled: true`、不给下载地址;`GET /plugins/market` 另把本机装着的内置插件由**本机清单**生成
条目并入列表 —— 远端索引没列它、列的是别的版本、甚至整个拉不到,它都在,也不会长出一个「更新」
(拉不到的原因放在回包的 `index_error` 里交给界面说)。界面上它标「内置」,不给装 / 更新 / 卸载。
...
| — | `plugin_instances.capability_status`(宿主能力最近一次对账的结果,插件页显示) |
| — | `provider_profiles.plugin_instance_id`、`provider_models.declared_capabilities`(插件生成连接,ADR 0020) |
