...
> 「那些形态在今天的代码里还处处成立吗」由后来的链条审计回答,最新一份是
> [2026-09-21 链条审计](2026-09-21-chain-audit.md):它沿着"同一份语义要在多处成立"的地方逐条
> 验证,并记录了三条**断在最后一环**的链(数据一直在,只是某一条路上看不见它)。数字规模见
> [architecture.md §0](architecture.md#0-规模速览实测),已按 1.4.2 重测。
