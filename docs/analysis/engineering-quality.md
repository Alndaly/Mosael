...
- 秘钥纪律：不导出钥匙串密码到聊天/脚本/日志/仓库；Windows 构建不获得 Apple 凭据（release.yml 第 228-232 行 env 按 `runner.os == 'macOS'` 条件注入）；描述文件按普通权限解码(它会嵌进每个发出去的包,不是秘密;此前的 `umask 077` 先让整个 `Mosael.app` 成了 `drwx------`,收窄后又让嵌入的描述文件成了 600,见 docs/validation/2026-09-23-release-1.4.3.md)。
