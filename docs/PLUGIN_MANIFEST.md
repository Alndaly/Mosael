...
**创意画板的工具格** —— 同一个节点也能放在画板上跑,上游的便签、图片接到字段上,产出落成右边
新的几格(见下面「在画板里长什么样」)。
...
每个工具同时也是**创意画板上的一种工具格**(`node:plugin.<包id>.<工具>`,见 ADR 0021):
用户从「添加 → 工具」或拉线菜单里放一格,表单就是上面那张节点表单;连进来的便签、文档、图片、
视频、音频、3D 场景可以接到字段上。你不用为画板写任何东西,但有几处声明决定了它在画板上好不好用:
...
- **`stores` 指向的那几格就是「授权会填的格子」。** 界面不把它们和 AppKey 摆成一样的主输入,而是收成
  一行「授权令牌」,只说每格填没填;展开「手动填写」照样能改 —— 已经有令牌的人不必走一遍授权。
