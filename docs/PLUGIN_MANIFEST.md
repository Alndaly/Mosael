...
- **不按另一个系统里的编号去取东西**:必填一个 `format: "external_id"` 字段的,或者不吃画板内容却收一个这种字段的,
  ...
  拖放,按编号取回是流程里的一步(列目录 → 取回)。画布上存着的这种工具格,宿主改成一张写明「这一步归工作流」的便签。
...
只在工具**真的**什么都没多做时才声明:它交出两个存下来的输出节点、一段文字、拿 alpha 当蒙版 —— 这些生成那一条
做不到,就不是同一件事。数输出节点时**只数生成会交回的那几个**:ComfyUI 插件 1.5.2 起不数预览节点(PreviewImage、
关了保存的视频合成写的是临时文件,生成跑完不交回、工具缺省也不交回),和生成收文件用同一个判据
(`graph.generation_nodes` / `collect_outputs`)—— 否则一张「保存 + 看一眼线稿」的 ControlNet 图在画板上就是两个入口。
工具的「也取回预览」是给搭流程、排查的人的开关,工作流里这个工具照旧在。清单里写死的工具不认这个键,它只属于
运行时报出的工具。
