...
describe("自由元素的尺寸不跟着素材比例走", () => {
  ...
  it("圆形蒙版的直径来自画幅的短边,不是素材铺满之后的短边", () => {
    ...
    // **圆里装多少源**也要对得上。导出是两次连续裁剪:源 →(铺满到 W×H 并裁)→(再中心裁成
    // min(W,H) 的正方)。换回源坐标就是 min(W,H)/铺满系数 = 1080/1.7778 = 607.5,居中。
    // 取源的中心正方 min(mw,mh)=1080 会**多取 1.78 倍**,于是圆里的内容被缩小同样的倍数。
    expect(ctx.drawImage).toHaveBeenCalledWith(layer.img, 236.25, 656.25, 607.5, 607.5, -540, -540, 1080, 1080);
  });
