...
describe("节点卡片上那一行摘要", () => {
  it("跳过素材 —— 它另有缩略图,裸 id 也不是给人看的", () => {
    const summary = outputSummary("ai_generate", { asset_id: "abc123", generation_id: "g1" });
    expect(summary).not.toContain("abc123");
  });

  it("取第一个非素材的产出", () => {
    expect(outputSummary("llm", { text: "模型回的话" })).toBe("模型回的话");
  });

  it("折行压成一行 —— 卡片上不该出现半截换行", () => {
    expect(outputSummary("llm", { text: "第一行\n\n  第二行" })).toBe("第一行 第二行");
  });

  it("没跑过就是空的", () => {
    expect(outputSummary("llm", undefined)).toBe("");
  });

  it("产出全是空值时不给一行空白", () => {
    // 给了空串的话,卡片上会多出一条什么都没有的灰条。
    expect(outputSummary("llm", { text: "", other: null })).toBe("");
  });
});
