...
describe("粘贴一组节点", () => {
  it("组内引用换成新 id,组外引用和原图不动", () => {
    ...
    const pasted = pasteNodes(graph, clip)!;
  ...
  it("循环体里的引用是体内的命名空间,不拿外层的换名表去改", () => {
    ...
    const loop = pasteNodes(graph, clip)!.graph.nodes.find((one) => one.type === "loop_foreach")!;
  ...
  it("start 不复制;只剩 start 时什么都不粘", () => {
    ...
    expect(pasteNodes(graph, { nodes: [node("start", "start")], edges: [] })).toBeNull();
  });
});
