...
import { Columns2, Grid2x2, Link2, Link2Off, Maximize2, X } from "lucide-react";
...
export function AssetCompareView({ assets, onClose }: { assets: Asset[]; onClose: () => void }) {
  ...
  const [grid, setGrid] = React.useState(() => images.length > 2);
  ...
  return (
    <div className="fixed inset-0 z-[140] grid grid-rows-[auto_minmax(0,1fr)] bg-[rgb(10_10_12/0.96)] backdrop-blur-[6px]">
      <div className="flex items-center gap-1.5 border-b border-border px-3 py-2">
        ...
        <Button variant="outline" size="sm" onClick={() => setGrid((v) => !v)} title={t(grid ? "mediaCompareTwoUp" : "mediaCompareGrid")}>
          {grid ? <Columns2 size={13} /> : <Grid2x2 size={13} />}
          {t(grid ? "mediaCompareTwoUp" : "mediaCompareGrid")}
        </Button>
        <Button
          variant="outline"
          size="sm"
