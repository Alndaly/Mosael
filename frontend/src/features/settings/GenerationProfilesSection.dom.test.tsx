...
function render_section() {
  ...
  return render(<GenerationProfilesSection profileId="p1" kind="image" />);
}
...
it("新建一份参数组:名字和 JSON 正文原样 POST 到这条连接下", async () => {
  ...
  fireEvent.click(screen.getByText("generationProfiles")); // 展开
  fireEvent.click(await screen.findByText("generationProfilesAdd"));
...
it("JSON 坏了不发请求 —— 那句报错该当场出,而不是绕一圈从服务端回来", async () => {
  ...
  fireEvent.click(screen.getByText("generationProfiles"));
  fireEvent.click(await screen.findByText("generationProfilesAdd"));
