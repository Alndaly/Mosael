 ...
 * 拖柄长什么样 —— **全应用只有这一份**。
 *
 * 压在栏与栏之间那条 1px 分割线的正中,8px 宽的热区里一根 36px 的短竖条,常显、悬停变主色。
 ...
 *
 * `gap` 默认 0:停靠的栏之间**只有一条 1px 分割线、没有缝**(剪辑、设置、插件、对话、生成、
 * 3D 场景都是这样),手柄压在线上。此前默认是 8,而设置/对话/生成三页的 grid 也还留着
 * `gap-2` —— 栏早就改成了分割线,那 8px 只剩一个作用:让手柄悬在离线 4px 的空白里,
 * 和别的页面看起来一处有缝、一处没缝。只有画布上的**浮动卡片**(工作流右侧那两块)
 * 之间真有缝,那里显式传 `gap: 8`。
 */
export function handleOffset(
  panelSize: number,
  { padding = 0, gap = 0, nextInset = 0 } = {},
): number {
